...
locals {
  ...
  seller_system_url = var.seller_system_url
...
// --- NETWORK --- //
locals {
  network_name           = var.network_name == "" ? "seller-network-${random_id.suffix.hex}" : var.network_name
  ...
  subnet_name            = var.subnet_name == "" ? "seller-network-subnet" : var.subnet_name
  ip_range_pods_name     = var.ip_range_pods_name == "" ? "seller-ip-range-pods" : var.ip_range_pods_name
  ip_range_services_name = var.ip_range_services_name == "" ? "seller-ip-range-services" : var.ip_range_services_name
...
locals {
  cluster_name = var.cluster_name == "" ? "seller-cluster" : var.cluster_name
}
...
locals {
  configs = {
    seller_adapter_config = {
      filename   = "seller-adapter-config.yaml"
      project_id = local.project_id
      seller_system = {
        url = local.seller_system_url
      }
      pubsub           = module.pubsub
      ondc_environment = var.ondc_environment
    }
    bpp_apis_config = {
      filename   = "bpp-apis-config.yaml"
      project_id = local.project_id
      subscriber = {
    ...
    callback_action_config = {
      filename   = "callback-action-config.yaml"
      project_id = local.project_id
      gateway = {
        url = local.gateway_url
      }
...
locals {
  service_accounts = [
    {
      name          = "seller-adapter",
      account_id    = "seller-adapter-service-account"
      display_name  = "Seller Adapter Service Account",
      k8s_name      = "seller-adapter-sa",
      k8s_namespace = "seller-adapter"
    },
    {
      name          = "bpp-apis",
      account_id    = "bpp-apis-service-account"
      display_name  = "BPP APIs Service Account",
      k8s_name      = "bpp-apis-sa",
      k8s_namespace = "bpp-apis"
    },
    {
      name          = "callback-action",
      account_id    = "callback-action-service-account"
      display_name  = "Callback Action Service Account",
      k8s_name      = "callback-action-sa",
      k8s_namespace = "callback-action"
    },
  ]
}

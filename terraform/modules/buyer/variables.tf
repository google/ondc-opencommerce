...
variable "spanner_instance_name" {
  ...
  default     = "buyer-ondc-spanner-instance"
}

variable "spanner_database_name" {
  type        = string
  description = "Spanner Database name"
  default     = "buyer-ondc-spanner-database"
}
...
variable "subscriber_url" {
  type        = string
  description = "Subscriber URL of the entity in the ONDC network eg. `https://abcstore.com/bap`"
}
...
variable "buyer_app_url" {
  type        = string
  description = "Buyer Application's URL for receiving buyer request eg. /on_search"
}
...
variable "spanner_display_name" {
  ...
  default     = "Buyer Spanner Instance"
}
...
variable "buyer_app_allow_hosts" {
  type        = list(string)
  description = "List of Allowed Hosts for Buyer App service. Default to `*` so that it can be called from the internet. It can be changed to IP address of the istio ingress gateway to secure the endpoint."
  default     = ["*"]
}
